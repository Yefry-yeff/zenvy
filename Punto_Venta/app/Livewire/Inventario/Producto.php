...
class Producto extends Component
{
    ...
    public function render()
    {
        ...
        $descuentosPorProducto = app(FacturaTotalesService::class)
            ->descuentosTotalesPorProducto($productos->getCollection()->pluck('id'));

        // Cargar códigos de barras y unidades de medida desde precio_has_venta
        $productos->getCollection()->transform(function($producto) use ($descuentosPorProducto) {
            $producto->descuento_total = $descuentosPorProducto->get($producto->id, 0);
