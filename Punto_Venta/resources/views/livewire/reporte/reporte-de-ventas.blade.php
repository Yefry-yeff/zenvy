                ...
                <table class="min-w-full text-xs border border-gray-200">
                        ...
                        <tr>
                            ...
                            <th class="px-2 py-3 text-right border-b cursor-pointer hover:bg-gray-200 w-24" wire:click="ordenar('sub_total_grabado')">
                                <div class="flex items-center justify-end space-x-1">
                                    <span class="text-xs font-semibold">Subtotal gravado</span>
                                    @if($ordenarPor === 'sub_total_grabado')
                                        <span class="text-blue-500">{{ $direccionOrden === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </div>
                            </th>
                            <th class="px-2 py-3 text-right border-b cursor-pointer hover:bg-gray-200 w-24" wire:click="ordenar('descuento_gravado')">
                                <div class="flex items-center justify-end space-x-1">
                                    <span class="text-xs font-semibold">Desc. gravado</span>
                                    @if($ordenarPor === 'descuento_gravado')
                                        <span class="text-blue-500">{{ $direccionOrden === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </div>
                            </th>
                            <th class="px-2 py-3 text-right border-b w-24">
                                <span class="text-xs font-semibold">Total gravado</span>
                            </th>
                            <th class="px-2 py-3 text-right border-b cursor-pointer hover:bg-gray-200 w-24" wire:click="ordenar('sub_total_exento')">
                                <div class="flex items-center justify-end space-x-1">
                                    <span class="text-xs font-semibold">Subtotal exento</span>
                                    @if($ordenarPor === 'sub_total_exento')
                                        <span class="text-blue-500">{{ $direccionOrden === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </div>
                            </th>
                            <th class="px-2 py-3 text-right border-b cursor-pointer hover:bg-gray-200 w-24" wire:click="ordenar('descuento_exento')">
                                <div class="flex items-center justify-end space-x-1">
                                    <span class="text-xs font-semibold">Desc. exento</span>
                                    @if($ordenarPor === 'descuento_exento')
                                        <span class="text-blue-500">{{ $direccionOrden === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </div>
                            </th>
                            <th class="px-2 py-3 text-right border-b w-24">
                                <span class="text-xs font-semibold">Total exento</span>
                            </th>
                            <th class="px-2 py-3 text-right border-b w-24">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($facturas as $factura)
                            <tr class="transition-colors duration-150 hover:bg-gray-50" wire:key="rv-{{ $factura->id }}">
                                ...
                                <td class="px-2 py-1 text-xs text-right">{{ number_format($factura->reporte_subtotal_gravado ?? 0, 2) }}</td>
                                <td class="px-2 py-1 text-xs text-right text-red-600">{{ number_format($factura->reporte_descuento_gravado ?? 0, 2) }}</td>
                                <td class="px-2 py-1 text-xs font-semibold text-right">{{ number_format($factura->reporte_gravado ?? 0, 2) }}</td>
                                <td class="px-2 py-1 text-xs text-right">{{ number_format($factura->reporte_subtotal_exento ?? 0, 2) }}</td>
                                <td class="px-2 py-1 text-xs text-right text-red-600">{{ number_format($factura->reporte_descuento_exento ?? 0, 2) }}</td>
                                <td class="px-2 py-1 text-xs font-semibold text-right">{{ number_format($factura->reporte_exento ?? 0, 2) }}</td>
                                <td class="px-2 py-1 text-xs font-semibold text-right text-red-700">{{ number_format($factura->reporte_descuento ?? 0, 2) }}</td>
                                <td class="px-2 py-1 text-xs text-right">{{ number_format($factura->reporte_isv ?? 0, 2) }}</td>
                    ...
                    <tfoot>
                        <tr class="text-xs font-bold bg-gray-200 border-t-2 border-gray-400">
                            ...
                            <td class="px-2 py-2 text-right text-gray-800">{{ number_format($totales->total_subtotal_gravado ?? 0, 2) }}</td>
                            <td class="px-2 py-2 text-right text-red-700">{{ number_format($totales->total_descuento_gravado ?? 0, 2) }}</td>
                            <td class="px-2 py-2 text-right text-gray-800">{{ number_format($totales->total_gravado ?? 0, 2) }}</td>
                            <td class="px-2 py-2 text-right text-gray-800">{{ number_format($totales->total_subtotal_exento ?? 0, 2) }}</td>
                            <td class="px-2 py-2 text-right text-red-700">{{ number_format($totales->total_descuento_exento ?? 0, 2) }}</td>
                            <td class="px-2 py-2 text-right text-gray-800">{{ number_format($totales->total_exento ?? 0, 2) }}</td>
                            <td class="px-2 py-2 text-right text-red-800">{{ number_format($totales->total_descuento ?? 0, 2) }}</td>
